...
export default async argv => {
  ...
  // Serve client.js/client.js.map from the site's status folder when present
  const altClientPath = path.join(argv.status, 'client.js')
  try {
    await fs.promises.access(altClientPath, fs.constants.R_OK)
    app.get(/^\/client\.js(\.map)?$/, (req, res, next) => {
      res.sendFile(path.join(argv.status, req.path.slice(1)), { dotfiles: 'allow', maxAge: '1h' }, err => err && next())
    })
  } catch {
    // no client.js in /status/ — bundled client from express.static below
  }
