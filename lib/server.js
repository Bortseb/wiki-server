...
export default async argv => {
  ...
  // Looks for an alternate client.js/client.js.map in the site's status folder, and serves those instead if it exists
  app.get(/^\/client\.js(\.map)?$/, (req, res, next) => {
    res.sendFile(path.join(argv.status, req.hostname, req.path.slice(1)), staticPathOptions, err => err && next())
  })
